...
with tab_event:
    ...
    st.caption(
        "Escolha um cenário de demonstração ou edite manualmente o JSON antes da análise."
    )
    ...
    scenarios = {
        "Rotor inclinado (cocked_rotor_2)": "cocked_rotor_2",
        "Desalinhamento": "desalinhado",
        "Desbalanceamento": "desbalanceado",
        "Condição normal": "normal",
        "Falha sem documentação": "falha_sem_documentacao",
    }
    ...
    demo_payload = base_payload.copy()

    demo_payload["fault"] = scenarios[selected_scenario]
    ...
    event_text = st.text_area(
        ...
        value=default_json,
        height=330,
        key=f"event_json_{selected_scenario}",
    )
...
with tab_chat:
    ...
    st.caption(
        "Consulte os procedimentos técnicos indexados no PostgreSQL/pgvector."
    )

    # Carrega automaticamente os faults disponíveis na base.
    # Isso evita manter a lista de seeds hardcoded no dashboard.
    stats_response = api_get("/api/v1/stats")

    seeded_faults = []
    if stats_response and stats_response.is_success:
        stats_data = stats_response.json()
        seeded_faults = [
            item.get("fault")
            for item in stats_data.get("top_faults", [])
            if item.get("fault")
        ]

    # Fallback para a demo caso a API ainda não tenha retornado estatísticas.
    fallback_faults = [
        "cocked_rotor_2",
        "desalinhado",
        "desbalanceado",
    ...
    # Estados normais não precisam aparecer como opção prescritiva.
    normal_states = {
        "normal",
        "healthy",
        "ok",
    ...
    demo_faults = [""] + available_faults

    fault_labels = {
        "": "Sem filtro de defeito",
        "cocked_rotor_2": "Rotor inclinado (cocked_rotor_2)",
        "cocked_rotor": "Rotor inclinado (cocked_rotor)",
        "desalinhado": "Desalinhamento",
        "desalinhamento": "Desalinhamento",
        "desbalanceado": "Desbalanceamento",
        "desbalanceamento": "Desbalanceamento",
        "imbalance": "Desbalanceamento (imbalance)",
        "misalignment": "Desalinhamento (misalignment)",
        "bearing_fault": "Falha de rolamento (bearing_fault)",
        "bearing": "Falha de rolamento (bearing)",
    }

    fault = st.selectbox(
        "Defeito / contexto",
        options=demo_faults,
        format_func=lambda value: fault_labels.get(
            value,
    ...
    default_questions = {
        "cocked_rotor_2": (
            "Qual procedimento devo seguir para diagnosticar "
            "e corrigir um rotor inclinado?"
        ),
        "cocked_rotor": (
            "Qual procedimento devo seguir para diagnosticar "
            "e corrigir um rotor inclinado?"
        ...
        "desalinhado": (
            "Qual procedimento de inspeção e correção "
            "devo seguir para desalinhamento?"
        ),
        "desalinhamento": (
            "Qual procedimento de inspeção e correção "
            "devo seguir para desalinhamento?"
        ...
        "desbalanceado": (
            "Como devo diagnosticar e corrigir "
        ...
        "bearing_fault": (
            "Qual procedimento devo seguir para diagnosticar "
            "e corrigir uma falha de rolamento?"
        ),
        "bearing": (
            "Qual procedimento devo seguir para diagnosticar "
            "e corrigir uma falha de rolamento?"
        ),
        "": "Qual procedimento de inspeção devo seguir?",
    }

    default_question = default_questions.get(
        fault,
        (
            f"Qual procedimento técnico devo seguir para diagnosticar "
            f"e corrigir a falha '{fault.replace('_', ' ')}'?"
            if fault
            else "Qual procedimento de inspeção devo seguir?"
        ),
    )
    ...
    question = st.text_area(
        ...
        height=100,
        key=f"chat_question_{fault or 'all'}",
    )
    ...
    if st.button(
        ...
        with st.spinner(
            "Buscando evidências na base vetorial..."
        ):
            response = api_post(
                "/api/v1/chat",
                json={
        ...
        if response and response.is_success:
            ...
            if steps:
                st.subheader("Procedimento sugerido")
